...
class TestValidOutput(unittest.TestCase):
    ...
    def test_save_winfetch(self):
        ...
        """
        with TemporaryDirectory() as temp_directory:
            response = self.config_saver.save_winfetch(temp_directory)
    ...
    def test_save_installed_programs(self):
        """
        Will test a single line in the installedprograms file that should always exist.
        """
        with TemporaryDirectory() as temp_directory:
    ...
    def test_save_global_python_packages(self):
        with TemporaryDirectory() as temp_directory:
            response = self.config_saver.save_global_python_packages(temp_directory)
    ...
    def test_save_choco_packages(self):
        with TemporaryDirectory() as temp_directory:
            response = self.config_saver.save_choco_packages(temp_directory)
    ...
    def test_save_vscode_extensions(self):
        with TemporaryDirectory() as temp_directory:
            response = self.config_saver.save_vscode_extensions(temp_directory)
            self.assertTrue(os.path.exists(os.path.join(temp_directory, 'vscode_extensions.txt')))
    ...
    def test_videos_directory_filenames(self):
        with TemporaryDirectory() as temp_directory:
            ...
            response = self.config_saver.save_videos_directory_filenames(temp_directory, videos_path)
    ...
    def test_save_file_associations(self):
        with TemporaryDirectory() as temp_directory:
            response = self.config_saver.save_file_associations(temp_directory)
            self.assertTrue(os.path.exists(os.path.join(temp_directory, 'file_associations.txt')))


    def test_save_drivers(self):
        with TemporaryDirectory() as temp_directory:
            response = self.config_saver.save_drivers(temp_directory)
    ...
    def test_save_systeminfo(self):
        with TemporaryDirectory() as temp_directory:
            response = self.config_saver.save_systeminfo(temp_directory)
...
class TestReturnType(unittest.TestCase):
    ...
    def test_command_runner_returns_str(self):
        response = self.config_saver._command_runner(['powershell.exe', 'Write-Output', 'Test'])
