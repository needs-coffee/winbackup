...
class TestValidOutput(unittest.TestCase):
    ...
    def test_add_item_raises_typeerror_on_invalid_key_type(self):
        ...
        with self.assertRaises(TypeError):
            response = self.config_agent.add_item(99, test_item)
    ...
    def test_add_item_raises_valueerror_with_invalid_configitemid(self):
        ...
        with self.assertRaises(ValueError):
            response = self.config_agent.add_item('invalidtestitemid', test_item)
    ...
    def test_get_global_config_returns_config(self):
        ...
        self.assertTrue(response['encryption_enabled'] == False)
    ...
    def test_validate_target_config(self):
        ...
        '10_documents': {'name': 'Documents', 'type': 'folder', 'path': '.', 'enabled': False, 'dict_size': '192m', 'mx_level': 9, 'full_path': False},
        }
    ...
    def test_validate_target_config_invalid_target_id(self):
        test_config = {
        'invalidconfigid': {'name': 'System Config', 'type': 'special', 'path': None, 'enabled': False, 'dict_size': '192m', 'mx_level': 9, 'full_path': False},
        '10_documents': {'name': 'Documents', 'type': 'folder', 'path': '.', 'enabled': False, 'dict_size': '192m', 'mx_level': 9, 'full_path': False},
        }
    ...
    def test_validate_target_config_invalid_target_invalid_configitem_key(self):
        test_config = {
        '01_config': {'name': 'System Config', 'type': 'special', 'path': None, 'enabled': False, 'dict_size': '192m', 'mx_level': 9, 'full_path': False},
        '10_documents': {'namesinvalid': 'Documents', 'type': 'folder', 'path': '.', 'enabled': False, 'dict_size': '192m', 'mx_level': 9, 'full_path': False},
        }
        self.assertFalse(self.config_agent.validate_target_config(test_config))
    ...
    def test_validate_target_config_invalid_target_invalid_mxlevel(self):
        test_config = {
        '01_config': {'name': 'System Config', 'type': 'special', 'path': None, 'enabled': False, 'dict_size': '192m', 'mx_level': 9, 'full_path': False},
        '10_documents': {'name': 'Documents', 'type': 'folder', 'path': '.', 'enabled': False, 'dict_size': '192m', 'mx_level': 99, 'full_path': False},
        }
        self.assertFalse(self.config_agent.validate_target_config(test_config))
    ...
    def test_validate_target_config_invalid_target_invalid_dictsize(self):
        test_config = {
        '01_config': {'name': 'System Config', 'type': 'special', 'path': None, 'enabled': False, 'dict_size': '192m', 'mx_level': 9, 'full_path': False},
        '10_documents': {'name': 'Documents', 'type': 'folder', 'path': '.', 'enabled': False, 'dict_size': 'xx192m', 'mx_level': 99, 'full_path': False},
        }
        self.assertFalse(self.config_agent.validate_target_config(test_config))
    ...
    def test_validate_target_config_invalid_target_path(self):
        test_config = {
        '01_config': {'name': 'System Config', 'type': 'special', 'path': None, 'enabled': False, 'dict_size': '192m', 'mx_level': 9, 'full_path': False},
        '10_documents': {'name': 'Documents', 'type': 'folder', 'path': None, 'enabled': False, 'dict_size': '192m', 'mx_level': 99, 'full_path': False},
        }
        self.assertFalse(self.config_agent.validate_target_config(test_config))
    ...
    def test_save_YAML_config(self):
        with TemporaryDirectory() as temp_directory:
            response = self.config_agent.save_YAML_config(os.path.join(temp_directory, 'test.yaml'))
    ...
    def test_save_YAML_config_invalid_given_directory(self):
        with TemporaryDirectory() as temp_directory:
            with self.assertRaises(IsADirectoryError):
                response = self.config_agent.save_YAML_config(os.path.join(temp_directory))


    def test_save_YAML_config_invalid_extension(self):
        with TemporaryDirectory() as temp_directory:
            with self.assertRaises(ValueError):
                response = self.config_agent.save_YAML_config(os.path.join(temp_directory, 'test.invalid'))
    ...
    def test_parse_YAML_config(self):
        ...
        with TemporaryDirectory() as temp_directory:
            save_path = self.config_agent.save_YAML_config(os.path.join(temp_directory, 'test.yaml'), test_config)
            global_config, target_config = self.config_agent.parse_YAML_config_file(save_path)
        with self.subTest("test globalconfig"):
            self.assertTrue(global_config['encryption_enabled'] == False)
...
class TestReturnType(unittest.TestCase):
    ...
    def test_save_YAML_config_returns_str(self):
        ...
        with TemporaryDirectory() as temp_directory:
            response = self.config_agent.save_YAML_config(os.path.join(temp_directory, 'test.yaml'), test_config)
        self.assertTrue(type(response) == str)
    ...
    def test_parse_YAML_config_returns_tuple_of_dicts(self):
        ...
        with TemporaryDirectory() as temp_directory:
            path = self.config_agent.save_YAML_config(os.path.join(temp_directory, 'test.yaml'), test_config)
