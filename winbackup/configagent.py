...
class ConfigAgent:
    def __init__(self, paths=None) -> None:
        ...
        # type - folder = simple folders, special = special backup function
        # path - backup target path
        # enabled - if the target will be backed up, default false for all
        # dict_size and mx_level - 7z dictionary size and compression level (ref 7z cli docs)
    ...
    def update_config_paths(self, paths: dict, config: dict = None) -> dict:
        """
        Base config does not have folder paths configured. Call with paths dictionary to update.
        Parameters:
        ...
        for id in config.keys():
            try:
                if updated_config[id]["type"].lower() == "folder":
                    ...
                    logging.debug(
                        f"Path updated -> {id} is {paths[id.split('_', 1)[1].lower()]}"
                    )
            except KeyError:
    ...
    def add_item(self, config_item_id: str, config_item: dict, config: dict = None) -> dict:
        ...
        if not type(config_item_id) == str:
            raise TypeError(
                f"config_item_id {config_item_id} not valid. Must be of type str, is of type {type(config_item_id)}"
            )
        if not type(config_item) == dict:
            raise TypeError(
                f"config_item {config_item} not valid. Must be of type dict, is of type {type(config_item)}"
            )
        ...
        if not re.match(r"(\d{2}_[a-z_]+)", config_item_id):
            raise ValueError(
                f"Invalid config_item_id: {config_item_id}. must be in the format 00_name or 00_long_name"
            )
        ...
        if new_config_item["name"] is None:
            raise ValueError("Name is required for config_item to be added to target_config")
        if new_config_item["type"] == "folder":
            if new_config_item["path"] is None:
                raise ValueError(
                    "path is required for config_item with type folder to be added to target_config"
                )
        ...
        try:
            ...
            logging.error(
                f"Could not add config_item {config_item_id} to config, exception - {e}"
            )
            logging.debug(traceback.format_exc())
            raise
    ...
    def get_target_config(self) -> dict:
        ...
        if not self._target_config:
            logging.warning(
                "Called before folder paths set. Base target_config used. Paths must be set before attempting backup."
            )
    ...
    def validate_target_config(self, config: dict = None) -> bool:
        ...
        for id in config:
            if not re.match(r"(\d{2}_[a-z_]+)", id):
                logging.warning(
                    f"Invalid Target ID for {id}. must be in the format 00_name or 00_long_name"
                )
                print(
                    f"Invalid Target ID for {id}. must be in the format 00_name or 00_long_name"
                )
        ...
        for id, config_item in config.items():
            ...
            if "mx_level" in config_item:
                if not 0 <= config_item["mx_level"] <= 9:
                    logging.error(
                        f"mx_level {config_item['mx_level']} for {id} not valid. Must be in range 0-9."
                    )
                    print(
                        f"mx_level {config_item['mx_level']} for {id} not valid. Must be in range 0-9."
                    )
            ...
            if "dict_size" in config_item:
                if not re.match(r"(^\d+[bkmg]?$)", config_item["dict_size"]):
                    logging.error(
                        f"dict_size {config_item['dict_size']} for {id} not valid. Must int followed by quantifier as b/k/m/g"
                    )
                    print(
                        f"dict_size {config_item['dict_size']} for {id} not valid. Must int followed by quantifier as b/k/m/g"
                    )
            ...
            if "path" in config_item and config_item["path"] is not None:
                    ...
                    if os.path.exists(path):
                        if os.path.isdir(path):
                            if not os.access(path, os.R_OK):
                                logging.warning(
                                    f"Target directory {path} for {id} exists but is not readable. Check access permissions."
                                )
                                print(
                                    f"Target directory {path} for {id} exists but is not readable. Check access permissions."
                                )
                            ...
                            logging.error(
                                f"Target path {path} for {id} exists but is not a directory. Backup target must be a directory."
                            )
                            print(
                                f"Target path {path} for {id} exists but is not a directory. Backup target must be a directory."
                            )
                        ...
                        logging.error(
                            f"Target path {path} does not exist but was specified for {id}. Check configuration."
                        )
                        print(
                            f"Target path {path} does not exist but was specified for {id}. Check configuration."
                        )
        ...
        if "output_root_dir" in global_config:
            if os.path.isdir(global_config["output_root_dir"]):
                if not os.access(global_config["output_root_dir"], os.W_OK):
                    logging.warning(
                        f"Output directory {global_config['output_root_dir']} is not writeable. Check access permissions."
                    )
                    print(
                        f"Output directory {global_config['output_root_dir']} is not writeable. Check access permissions."
                    )
                ...
                logging.error(
                    f"Output Directory path {global_config['output_root_dir']} does not exist. Check configuration."
                )
                print(
                    f"Output Directory path {global_config['output_root_dir']} does not exist. Check configuration."
                )
    ...
    def save_YAML_config(
        ...
        if not target_config:
            ...
            if not target_config:
                ...
                logging.warning(
                    "Saved config template does not have correct user paths. It is recommended to update config paths first."
                )
        ...
        combined_config = {"global": global_config, "backup_targets": target_config}
    ...
    def parse_YAML_config_file(self, path: str) -> tuple:
        ...
        if len(merged_global_config["encryption_password"]) != 0:
            logging.debug(
                "Encryption password from configfile is not empty, enable encryption"
            )
