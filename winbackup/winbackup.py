...
class WinBackup:
    def __init__(self, log_level) -> None:
        ...
        self.hyperv_paths_script = os.path.join(real_path, "scripts", "hyperv_paths.ps1")

        self.config_saver.set_videos_directory_path(
            self.config_agent._target_config["14_videos"]["path"]
        )
    ...
    def cli_get_output_root_path(self) -> str:
        ...
        loop_limit = 5
        while 1:
            print(Fore.GREEN + " Backup output directory: " + Style.RESET_ALL, end="")
            ...
            if os.path.isdir(reply):
                ...
                print(
                    Fore.RED
                    + " Output directory must be a real path. Ctrl+C to exit."
                    + Style.RESET_ALL
                )
                loop_limit -= 1
    ...
    def _start_logger(self, log_level) -> None:
        ...
        if log_level == logging.DEBUG:
            ...
            log_handler = logging.FileHandler(
                os.path.join(os.getcwd(), "winbackup.log"), mode="w", encoding="utf-8"
            )
        else:
    ...
    def _redirect_logger(self, out_path, log_level) -> None:
        ...
        if log_level == logging.DEBUG:
            ...
            shutil.move(
                os.path.join(os.getcwd(), "winbackup.log"),
        ...
        log_handler = logging.FileHandler(
            os.path.join(out_path, "winbackup.log"), mode="a", encoding="utf-8"
        )
        formatter = logging.Formatter(log_format)
        log_handler.setFormatter(formatter)
        logger.addHandler(log_handler)
    ...
    def _plex_server_running(self) -> bool:
        response = self._command_runner(
            ["powershell.exe", "Get-Process", '"Plex Media Server"']
        ).split("\r\n")[0]
    ...
    def _yes_no_prompt(question: str) -> bool:
        while 1:
            ...
            if reply in {"yes", "ye", "y"}:
                ...
                print(
                    Fore.RED + " Only yes or no permitted. Ctrl+C to exit." + Style.RESET_ALL
                )
    ...
    def cli_header(
        self, output_path: str, output_folder_name: str, start_time: datetime
    ) -> None:
        print(
            Fore.BLACK
            + Back.WHITE
        ...
        print(
            ...
            + Style.RESET_ALL
            + f" {self._create_filename('example')}"
        )
        print(Fore.GREEN + " Output folder         : " + Style.RESET_ALL + f" {output_path}")
        print(
            Fore.GREEN
        ...
        print()
        print(
            " A new folder will be created in the specified output folder with the above archive folder name pattern."
        )
    ...
    def cli_get_password(self) -> str:
        ...
        while 1:
            print(
                Fore.GREEN
                + " > Password for encryption (leave blank to disable) : "
                + Style.RESET_ALL,
                end="",
            )
            ...
            if len(passwd) == 0:
                ...
                print(
                    Fore.GREEN + " > Confirm encryption password: " + Style.RESET_ALL, end=""
                )
                passwd_confirm = getpass.getpass(prompt="")
                if passwd == passwd_confirm:
                    logging.info("Archives will be encrypted.")
                    print(
                        " Passwords match. 7z archives will be AES256 encrypted with the given password."
                    )
                    print(
                        " The archive headers will be encrypted (Filenames will be encrypted)."
                    )
                    break
                else:
                    print(
                        Fore.RED + " > passwords don't match - try again. " + Style.RESET_ALL
                    )
        print()
        return passwd
    ...
    def cli_config_summary(self, config: dict, passwd: str, path_created: bool) -> None:
        print(
            Fore.BLACK + Back.WHITE + " ** CONFIG SUMMARY ** " + Style.RESET_ALL + Fore.GREEN
        )
        for key, target in sorted(config.items()):
            print(
                f" Backup {target['name']:<14} - {'Yes' if target['enabled']==True else 'No'}"
            )
        ...
        if len(passwd) <= 12 and len(passwd) != 0:
            ...
            logging.debug(
                f"The given password is short ({len(passwd)} chars). Consider a longer password."
            )
        if not path_created:
            print(
                Fore.YELLOW
                + " !! CAUTION - Output directory already exists - Contents may be destroyed if you proceed."
                + Style.RESET_ALL
            )
            logging.info(
                "Output directory already exists - Contents may be destroyed if you proceed."
            )
        if "30_plexserver" in config:
            if config["30_plexserver"]["enabled"] and self._plex_server_running():
                print(
                    Fore.YELLOW
                    + " !! CAUTION - Plex Media Server is running - Recommend stopping Plex Server before backing up."
                    + Style.RESET_ALL
                )
                logging.info(
                    "Plex Media Server is running - Recommend stopping Plex Server before backing up."
                )
        if "32_hypervvms" in config:
            if config["32_hypervvms"]["enabled"]:
                print(
                    Fore.YELLOW
                    + " !! CAUTION - HyperV has been enabled - Please check VMs are stopped before running."
                    + Style.RESET_ALL
                )
                logging.info(
                    "HyperV has been enabled - Please check VMs are stopped before running."
                )
        if self._hyperv_possible() and not self.check_if_admin():
            print(
                Fore.CYAN
                + " -- INFO - HyperV detected on system. To backup HyperV run winbackup as admin."
                + Style.RESET_ALL
            )
        ...
        print(
            Fore.CYAN
            + " -- INFO - Archives produced are split into 4092Mb volumes (FAT32 limitation)."
            + Style.RESET_ALL
        )
        print()
    ...
    def remove_existing_archive(filename, path):
        paths_to_remove = [
            os.path.join(path, file) for file in os.listdir(path) if file.startswith(filename)
        ]
        logging.debug(
            f"existing archives to be removed before backing up: {len(paths_to_remove)}"
        )
    ...
    def cli_exit(self, out_path: str, start_time: datetime) -> None:
        ...
        logging.info(
            f"Total backup size {backup_size} ({humanize.naturalsize(backup_size, True)})"
        )
        print(
            Fore.WHITE + f" Completed in {humanize.naturaldelta(duration)}" + Style.RESET_ALL
        )
        print(Fore.WHITE + f" Total backup size {humanize.naturalsize(backup_size, True)}")
        print(Fore.GREEN + " Backups done! " + Style.RESET_ALL)
    ...
    def interactive_config_builder(self, target_path: str, all_selected: bool = False) -> None:
        ...
        print(Fore.GREEN + " Interactive configuration builder" + Style.RESET_ALL)
        if not target_path:
        ...
        print(
            Fore.GREEN
            + " Generated configuration file will be saved to: "
            + Style.RESET_ALL
            + f"{target_path}"
        )
        print()
        ...
        try:
            ...
            print(
                Fore.RED
                + " XX - Could not save configuration file. See logs. Exiting."
                + Style.RESET_ALL
            )
            sys.exit(1)
    ...
    def run_from_config_file(self, path: str, quiet: bool = False) -> None:
        if path:
            if os.path.exists(path) and path.lower().endswith((".yaml", ".yml")):
                ...
                logging.critical(
                    f"Config file does not exist or is not a YAML file at {path}. Exiting."
                )
                print(
                    Fore.RED
                    + f" XX - Config file does not exist or is not a YAML file at {path}. Exiting."
                    + Style.RESET_ALL
                )
                sys.exit(1)
        else:
            logging.critical("Must give path to configuration file, none given. Exiting.")
            print(
                Fore.RED
                + " XX - Must give path to configuration file. Exiting."
                + Style.RESET_ALL
            )
            sys.exit(1)
        ...
        try:
            ...
            print(
                Fore.RED
                + " XX - Could not load config from file. See logs. Exiting."
                + Style.RESET_ALL
            )
            sys.exit(1)
    ...
    def cli(
        ...
        if not root_path:
            ...
            if not os.path.isdir(root_path):
                print(
                    Fore.RED
                    + " Output directory must be a real path. Exiting."
                    + Style.RESET_ALL
                )
        ...
        (
            ...
            self.path_created,
        ) = self._create_output_directory(self.config_agent.output_root_dir)
        ...
        if self._recursive_loop_check(self.output_path, self.config_agent.target_config):
            print(
                Fore.RED
                + " XX - Output path is a child of a path that will be backed up. This will case an infinite loop. Choose a different path."
                + Style.RESET_ALL
            )
            logging.critical(
                "Output path is a child of path that will be backed up. This will case an infinite loop. Choose a different path."
            )
            print(" Aborted. Exiting.")
            sys.exit(0)

        self.cli_config_summary(
            self.config_agent.target_config,
            self.config_agent.encryption_password,
            self.path_created,
        )
